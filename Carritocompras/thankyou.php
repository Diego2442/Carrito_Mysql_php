<?php
  ...
  //esta variable ayudaria para formlar el no repetir usuario $email = $_POST["email"];
  ...
  for ($i=0; $i < count($arreglo) ; $i++) {
    ...
    $conexion ->query("insert into usuario (nombre,telefono,email,password) 
        ...
        //tratar de no repetir correo
      /*  $nuevo_usuario= $conexion->query("select email from usuario where email=".'$email');
        if (mysqli_num_rows($nuevo_usuario)>0) { echo $conexion-> rollback();
          # code...
        }else{$conexion ->query("insert into usuario (nombre,telefono,email,password) 
        values(
              '".$_POST['c_fname']."".$_POST['c_lname']."',
              '".$_POST['c_phone']."',
              '".$_POST['c_email_address']."',
    ...
    for ($i=0; $i < count($arreglo) ; $i++) {
      ...
      //se utiliza para descontar los productos del inventario
...
//$conexion-> begin_transaction();  
$conexion -> query("insert into pedidos(pais,direccion,region,postal,id_venta) values(
          '".$_POST['c_country']."',
          '".$_POST['c_address']."',
